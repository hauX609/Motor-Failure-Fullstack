...
def _create_prediction_side_effects(motor_id: str, result: dict) -> None:
    """Create alerts and persist status updates for a successful prediction."""
    if result.get('alert_needed') and result.get('predicted_status') != 'Optimal':
        message = (
            f"Status changed from {result.get('previous_status')} to {result.get('predicted_status')}. "
            f"Predicted RUL: {result.get('predicted_rul', 0):.0f}"
        )
        try:
...
def health_check():
    ...
    try:
        ...
        if db_healthy and runtime_state['trained_model_loaded']:
            status = 'healthy'
        elif db_healthy and runtime_state['ready']:
            status = 'degraded'
        else:
            status = 'unhealthy'
        ...
        return jsonify({
            'status': status,
            'timestamp': now_iso(),
            'model_loaded': runtime_state['trained_model_loaded'],
            'prediction_ready': runtime_state['ready'],
            'prediction_mode': runtime_state['mode'],
            ...
            'feature_count': runtime_state['feature_count']
        })
...
def predict(motor_id):
    ...
    try:
        ...
        _create_prediction_side_effects(motor_id, result)
    ...
    try:
        ...
        if ml_manager.fallback_mode:
            for motor_id in motor_list:
                try:
                    result = prediction_service.predict_single_motor(
                        motor_id,
                        valid_motors[motor_id],
                        previous_statuses.get(motor_id, 'Optimal'),
                    )
                    results.append(result)
                    status_updates.append((motor_id, result['predicted_status']))
                    if result.get('alert_needed') and result['predicted_status'] != 'Optimal':
                        alerts_to_create.append({
                            'motor_id': motor_id,
                            'severity': result['predicted_status'],
                            'message': (
            ...
            # Prepare batch prediction with the trained model
            processed_sequences = []
