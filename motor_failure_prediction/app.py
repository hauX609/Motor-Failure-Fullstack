...
def initialize_runtime():
    ...
    elif ml_manager.has_trained_model():
        logger.info("Initializing SHAP explainers...")
        ml_manager.initialize_explainers()
    else:
        logger.warning(
            "Prediction runtime is operating in fallback mode: %s",
            ml_manager.fallback_reason,
        )
    ...
    logger.info(
        "Prediction runtime ready: mode=%s trained_model_loaded=%s fallback_active=%s",
        runtime_state['mode'],
        runtime_state['trained_model_loaded'],
